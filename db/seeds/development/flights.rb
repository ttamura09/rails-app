...
0.upto(30) do
  ...
  10.upto(39) do |flight_num|
    ...
    1.upto(3) do |alnum|
      ...
      # 以下、所要時間と値段設定----------
      if orig_dest_ids.include?(0) # 新千歳←→
        if orig_dest_ids.include?(1) || orig_dest_ids.include?(2) || orig_dest_ids.include?(3) # 羽田・成田・中部
          price = 8000
          duration = 120
        elsif orig_dest_ids.include?(4) || orig_dest_ids.include?(5) # 関空・伊丹
          price = 11000
          duration = 140
        elsif orig_dest_ids.include?(6) # 福岡
          price = 13000
          duration = 170
        else
          # 那覇
